...
class Controller_Passoffice2s extends Controller_Template
{
	...
	*/
	public function action_issue()
	{
		...
		//$this->redirect('Passoffice2s/edit/0/issue');// параметр 0 - значит, новый гость
		$this->redirect('Passoffice2s/edit/0/issue');// параметр 0 - значит, новый гость
	}
	...
	*/
	public function action_save()
	{
		...
		$this->redirect('Passoffice2s/edit/0/issue');
			break;
			...
			*/
			case 'forceexit':// ручная отметка о выходе
				...
				$this->redirect('Passoffice2s/edit/'.Arr::get($_POST,'id_pep', 0).'/guest_mode');	
		...
		//$this->redirect('Passoffice2s');
		$this->redirect('Passoffice2s/edit/' . $id);
	}
	...
	*/
	...
	public function action_edit()
	{
		...
		$id=$this->request->param('id');// кого редактируем
		...
		if($id == 0) {    //если id=0, то создаем нового пипла
			$contact->is_active=1;
			$contact->id_org=Arr::get(Auth::instance()->get_user(), 'ID_ORGCTRL');// при создании нового пипла ему присваивается id_org, которым управляет текущий пользователь
		} else {
		...
		//$org_tree = Model::Factory('Company')->getOrgList();// получить список организаций для построения дерева организаций.
		$org_tree = Model::Factory('Company')->getOrgListForOnce($this->idOrgGuest);// для гостя получаю единственную организацию, чтобы построить дерево.
		...
		$this->template->content = View::factory('edit')
			...
			->bind('mode', $mode)
			->bind('idOrgGuest', $this->idOrgGuest)
			->bind('idOrgGuestArchive', $this->idOrgGuestArchive)
	...
	public function _action__view()
	{
		...
		if (!$contact) $this->redirect('Passoffice2s');
	...
	public function action_history()
	{
		...
		if (!$contact) $this->redirect('Passoffice2s');//если контакта нет, то перенаправление на список контактов 
		$data = History::getHistory($id);// беру историю для указанного контакта историю (контроллер History.php, метод getHistory($user))
		
		
		
		$topbuttonbar=View::factory('topbuttonbar', array(
		    'id_pep'=> $id,
		    '_is_active'=> 'history',
		))
		;
		
		$this->template->content = View::factory('history')//вызываю вью contacts/history.php
			->bind('contact', $contact)
			->bind('data', $data)
			->bind('id', $id)
	...
	public function _action_addgrz()
	{
		...
		$this->template->content = View::factory('grz')
			->bind('contact', $contact)
			->bind('anames', $anames);
	}
	...
	*/
	public function action_cardlist()
	{
		...
		$this->template->content = View::factory('cardlist')
			->bind('contact', $contact)
			->bind('cards', $cards)
			->bind('alert', $fl)
			->bind('id', $id)
	...
	*/
	public function action_acl()
	{
		...
		if ($id != "0" && !$contact) $this->redirect('Passoffice2s');//если что не так - переход на корень
		...
		$this->template->content = View::factory('acl')
			->bind('contact', $contact)
			->bind('alert', $fl)
			->bind('arrAlert', $arrAlert)
			->bind('mode', $mode)
	...
	*/
	public function action_saveACL()
	{
		...
		$alert=__('Passoffice2s.resultUpdateAcl', array(':addAclOkCount'=>$addAclOkCount,':delAclOkCount'=>$delAclOkCount ));		
		$arrAlert[]=array('actionResult'=>0, 'actionDesc'=>$alert);		
		Session::instance()->set('arrAlert',$arrAlert);
		...
		$this->redirect('Passoffice2s/acl/'.$id_pep);
	}
	...
	public function _action_reload()
	{
		...
		$this->redirect('Passoffice2s/cardlist');
	}
	...
	public function action_config()
	{
		...
		$this->template->content = View::factory('config')
		->bind('alert', $fl)
	...
	public function action_saveconfig()
	{
		...
		$this->redirect('Passoffice2s/config');
