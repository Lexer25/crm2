...
public function getListNowOrder($id_pep, $mode = null, $user_role = null, $buro_filter = '', $show_all = false)
{
    ...
    if ($mode === 'guest_mode') {
        if ($show_all) {
            ...
            $sql .= ' AND (CAST(g2.timeplan AS DATE) = CURRENT_DATE 
                OR (CAST(g2.timeplan AS DATE) < CURRENT_DATE 
    ...
    if (!empty($result)) {
        ...
        // Запрос к базе bucfg для получения информации о бюро
        $buro_sql = 'SELECT 
                        bg.id_pep,
		...
		//echo Debug::vars('135', $buro_results);exit;
    ...
    Log::instance()->add(Log::DEBUG, '149 TIME EXECUTE t1', array('t1'=>(microtime(true)-$start)));
    return $result;
}
