...
//order_mode - резервный режим. предполагает работу только со списком ранее разрешенных гостей. Не реализован
...
//echo Debug::vars('131', $topbuttonbar);
...
<div class="onecolumn">
	...
	<div class="content">
		...
		<form action="passoffices/save" method="post" onsubmit="return validate()">
			...
			<fieldset>
				...
				$cardList=$guest->getTypeCardList(1);
							 foreach ($cardList as $key1=>$value)
							 {
								 ...
								 echo $card->id_card.' ('.$card->id_card_on_DEC.')';
								echo '<br>';
			...
			<?php // если карт нет, то вывожу форму для добавления карты
					...
					switch (Kohana::$config->load('system')->get('regFormatRfid')){
						case 0:
							switch (Kohana::$config->load('system')->get('baseFormatRfid')){
								...
								break;
								case 1:
									echo __('contact.wait_001A_number');
								break;
								default:
									echo __('contact.wait_not_point_number');
								break;
							}							
						break;
						case 2:
							echo __('contact.wait_dec10_number');
						break;
						default:
							echo __('contact.check_reg_device_setting');
						break;
				...
				?>			
						...
						<?php 
							...
							echo '<br><label for="note">'.__('passoffices.note').'</label><br>';
			...
			switch($mode){
				...
				case 'guest_mode'://просмотр гостя с картой, можно сделать отметку о выходе
					...
					//echo Form::button('forceexitUpdate', __('guest.update'));
					echo Form::submit('forceexitUpdate', __('guest.update'), array('class'=>'column_right'));
		...
		echo Form::open('passoffices');
			//echo Form::submit(null, __('button.backtolist'), array('onclick'=>'location.href='.URL::base().'passoffices'));
		echo Form::close();
			
		echo Form::open('passoffices/testAddGuest');
